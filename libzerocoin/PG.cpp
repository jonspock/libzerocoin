...
void CalculateParams(ZerocoinParams &params, CBigNum N, string aux, uint32_t securityLevel) {
    ...
    for (uint32_t count = 0; count < MAX_ACCUMGEN_ATTEMPTS && params.accumulatorParams.accumulatorBase.isOne();
         count++) {
...
IntegerGroupParams deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen) {
    ...
    result.u_inner_prod =
        calculateGroupGenerator(seed, pSeed, qSeed, result.modulus, result.groupOrder, ZKP_N + ZKP_PADS + 2);
...
CBigNum generateIntegerFromSeed(uint32_t numBits, uint256 seed, uint32_t *numIterations) {
    ...
    for (uint32_t count = 0; count < iterations; count++) {
        ...
        result += CBigNum(calculateHash(ArithToUint256(UintToArith256(seed) + count))) *
                  CBigNum(2).pow(count * HASH_OUTPUT_BITS);
    }

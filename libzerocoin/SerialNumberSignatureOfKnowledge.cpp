                                                                                      ...
                                                                                      sprime(p->zkp_iterations)
{
    ...
    if (IntegerModModulus<COIN_COMMITMENT_MODULUS>::getModulus() != IntegerModModulus<SERIAL_NUMBER_SOK_COMMITMENT_GROUP>::getModulus()) {
    ...
    for (uint32_t i = 0; i < params->zkp_iterations; i++) {
        ...
        if (challenge_bit) {
            ...
            sprime[i] = v_expanded[i] - (commitmentToCoin.getRandomness() *
                                            (b^(r[i] - coin.getRandomness())).getValue());
        }
    }
}

inline CBigNum SerialNumberSignatureOfKnowledge::challengeCalculation(const CBigNum& a_exp,
                                                                      const CBigNum& b_exp,

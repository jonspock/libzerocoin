...
bool SerialNumberSoKProof::BatchVerify(std::vector<SerialNumberSoKProof> &proofs) {
    ...
    for(unsigned int w=0; w<proofs.size(); w++)
    {
        ...
        for(int i=0; i<m; i++) {
            if( ComA[i] < CBigNum(0) || ComA[i] > p )
              throw std::runtime_error("wrong value for ComA");
            if( ComB[i] < CBigNum(0) || ComB[i] > p )
              throw std::runtime_error("wrong value for ComB");
            if( ComC_[i] < CBigNum(0) || ComC_[i] > p )
              throw std::runtime_error("wrong value for ComC");
        }
        ...
        for(int i=0; i<m1dash; i++)
            if( polyComm->Tf[i] < CBigNum(0) || polyComm->Tf[i] > p )
                throw std::runtime_error("wrong value for Tf");

        for(int i=0; i<m2dash; i++)
            if( polyComm->Trho[i] < CBigNum(0) || polyComm->Trho[i] > p )
                throw std::runtime_error("wrong value for Trho");
        ...
        for(int i=0; i<ndash; i++)
            if( polyComm->tbar[i] < CBigNum(0) || polyComm->tbar[i] > q )
                throw std::runtime_error("wrong value for tbar");
        ...
        for(int i=0; i<M1; i++)
            for(int j=0; j<N1; j++) {
                if( innerProduct->final_a[i][j] < CBigNum(0) || innerProduct->final_a[i][j] > q )
                  throw std::runtime_error("wrong value for final_a");
                if( innerProduct->final_b[i][j] < CBigNum(0) || innerProduct->final_b[i][j] > q )
                  throw std::runtime_error("wrong value for final_b");
            }
    ...
    if(test != comTest) {
      throw std::runtime_error("BatchVerify failed: different test and comTest\n");
      return false;
    }

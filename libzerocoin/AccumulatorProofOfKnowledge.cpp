    ...
    Accumulator& a) : params(p)
{
    ...
    **/
    ...
    /// The proves chooses \f$ r_&gamma;, r_&phi;, r_&sigma;, r_&epsilon;, r_&xi;, r_&eta;, r_&zeta;, r_&beta;, r_&delta; \f$
    ...
    hasher << *params << sg.getValue() << sh.getValue() << g_n.getValue() << h_n.getValue()
           << commitmentToCoin.getCommitmentValue() << C_e << C_u << C_r
...
bool AccumulatorProofOfKnowledge::Verify(const Accumulator& a, const CBigNum& valueOfCommitmentToCoin) const
{
    ...
    CHashWriter hasher(0, 0);
    hasher << *params << sg.getValue() << sh.getValue() << g_n.getValue() << h_n.getValue()
           << valueOfCommitmentToCoin << C_e.getValue() << C_u << C_r << st_1 << st_2 << st_3 << t_1 << t_2 << t_3 << t_4;

...
CoinSpend::CoinSpend(const ZerocoinParams* p, const PrivateCoin& coin, Accumulator& a, const uint32_t checksum, const AccumulatorWitness& witness, const uint256& ptxHash) : accChecksum(checksum), ptxHash(ptxHash), coinSerialNumber((coin.getSerialNumber())), accumulatorPoK(&p->accumulatorParams), commitmentPoK(&p->serialNumberSoKCommitmentGroup, &p->accumulatorParams.accumulatorPoKCommitmentGroup)
{
    ...
    Commitment fullCommitmentToCoinUnderSerialParams =
      commit<SERIAL_NUMBER_SOK_COMMITMENT_MODULUS,
             SERIAL_NUMBER_SOK_COMMITMENT_GROUP>
        (p->serialNumberSoKCommitmentGroup.g,
         p->serialNumberSoKCommitmentGroup.h,
         coin.getPublicCoin().getValue());
    
    this->serialCommitmentToCoinValue = fullCommitmentToCoinUnderSerialParams.getCommitmentValue();

    Commitment fullCommitmentToCoinUnderAccParams = commit<ACCUMULATOR_POK_COMMITMENT_MODULUS,
                                                           ACCUMULATOR_POK_COMMITMENT_GROUP>
      (p->accumulatorParams.accumulatorPoKCommitmentGroup.g,
       p->accumulatorParams.accumulatorPoKCommitmentGroup.h,
       coin.getPublicCoin().getValue());
    ...
    this->accumulatorPoK = AccumulatorProofOfKnowledge(&p->accumulatorParams,
    ...
    this->serialNumberSoK = SerialNumberSignatureOfKnowledge(p, coin,
                                                             fullCommitmentToCoinUnderSerialParams, signatureHash());
}
